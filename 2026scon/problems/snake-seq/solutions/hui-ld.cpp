...
bool Inc(int p1, int p2, int p3){
    return 1.L * (Y[p2] - Y[p1]) / (X[p2] - X[p1]) + eps < 1.L * (Y[p3] - Y[p2]) / (X[p3] - X[p2]);
}

bool Dec(int p1, int p2, int p3){
    return 1.L * (Y[p2] - Y[p1]) / (X[p2] - X[p1]) > 1.L * (Y[p3] - Y[p2]) / (X[p3] - X[p2]) + eps;
}
...
void Solve(int p){
    ...
    for(int a=0, b=0, mx=INF; b<nxt.size(); b++){
        while(a < prv.size() && Inc(prv[a], p, nxt[b])) mx = max(mx, D[prv[a]][p][1]), a++;
        D[p][nxt[b]][0] = mx + 1;
    }
    reverse(prv.begin(), prv.end());
    reverse(nxt.begin(), nxt.end());
    for(int a=0, b=0, mx=INF; b<nxt.size(); b++){
        while(a < prv.size() && Dec(prv[a], p, nxt[b])) mx = max(mx, D[prv[a]][p][0]), a++;
        D[p][nxt[b]][1] = mx + 1;
    }
}
