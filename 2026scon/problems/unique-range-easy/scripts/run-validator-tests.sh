...
wine files/val_urange.exe < files/tests/validator-tests/01 2>> validator-tests.log
if [ "$?" -ne "0" ]; then
...
echo "Validator comment:" 1>> validator-tests.log
wine files/val_urange.exe < files/tests/validator-tests/02 2>> validator-tests.log
if [ "$?" -eq "0" ]; then
