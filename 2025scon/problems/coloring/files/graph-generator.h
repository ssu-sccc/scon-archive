...
namespace graph_gen {
    ...
    make_undirected_simple_graph(int n, int m, std::vector<std::pair<int, int>> used = {}) {
        ...
        if (n <= 5000) {
            for (int i = 1; i <= n; i++)
                for (int j = i + 1; j <= n; j++)
                    if (!std::binary_search(used.begin(), used.end(), std::make_pair(i, j)))res.emplace_back(i, j);
