...
fn main() {
    ...
    let c = |n: usize, r: usize| -> usize {
        (0..r).fold(1, |acc, i| acc * (n.saturating_sub(i)) / (i + 1))
    };

    let total_ones = ones.len();
    ...
    let total_zeros = zeros.len();
    let over_o1 = zeros.iter().filter(|&&x| x > o1).count();
    let over_o2 = zeros.iter().filter(|&&x| x > o2).count();
    let over_o3 = zeros.iter().filter(|&&x| x > o3).count();

    let total_twos = twos.len();
    ...
    let chooable_ones = total_ones.saturating_sub(1);
    ...
    let choosable_ones = total_ones.saturating_sub(2);
    ans += over_o2 * choosable_ones;

    // two non-largest elements has been chosen.
    let choosable_ones = total_ones.saturating_sub(1);
