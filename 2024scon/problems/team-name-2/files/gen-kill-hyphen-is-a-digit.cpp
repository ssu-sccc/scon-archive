...
int main(int argc, char* argv[]){
    ...
    int len = rnd.next(8, 10), digit = rnd.next(0, len), hyphen = len - digit;
    string pass_str(hyphen, '-');
    for(int i=0; i<digit; i++) pass_str += char('0' + rnd.next(0, 9));
    shuffle(pass_str.begin(), pass_str.end());
    V.insert(V.begin()+pos, pass_str);
