...
int main(int argc,char* argv[])
{
    ...
    else { M = rnd.next(N-1, int(min(300'000ll, 1ll*N*(N-1)/2))); }
 
    vector<pair<int, int>> edges = graph::make_simple_connected(N, M);
    graph::shuffle_edge(edges);
 
    ll K = rnd.next(0ll, (1ll << 60)-1);

    cout << N << " " << M << "\n";
    for(auto [X, Y] : edges) {
        auto Z = rnd.next(0ll, (1ll << 60)-1);
        if(X == N || Y == N) Z = Z & K;
        cout << X << " " << Y << " " << Z << "\n";
    }
    return 0;
